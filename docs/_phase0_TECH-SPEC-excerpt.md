===== FILE: docs\TECH-SPEC-PIPELINE.md =====
bytes=11631
# Technical Spec: Deterministic Image Pipeline & Area �+' Bdft

Status: SPEC A� 2026-08-27 A� Gate A freeze 2026-08-31 A� TyTech / SlabUploader
...
| bdft | sqft A- thickness (inches) | 2 decimals |
| Price | species $/bdft A- bdft, or override | 2 decimals, half-up |
...
Settings navigation. Changing any knob **immediately re-runs** A3.4 on the current
...
- User confirms the axis matches the slab�?Ts prevailing length.
...
```
sqft = (mask_pixel_count / scaleA�) / 144
```
...
- Capture: `recommendation = bdft A- species.price_per_bdft`, pre-fills price.
...
bdft = 0.64 A- 1.5 = **0.96**.

### TV-4 Width samples
96" length axis, stations every 6". Widths perpendicular to the confirmed axis. Listing gets min, max, avg.

### TV-5 Angled end
One square end, one 45A� cut. Length axis follows prevailing length, not the diagonal. Widths stay perpendicular after user confirm.
...
5. bdft = sqft A- thickness (inches). Not rounded up to whole board feet.
...
| Portable Call 1�+'Call 2 context (full resend default) | ARCHITECTURE A9, PROMPTS |
...
| Online-only capture (offline deferred) | AGENTS A2, ARCHITECTURE A4 |
