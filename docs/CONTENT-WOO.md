...
### 1.2 Optional LLM refinement (FR17, toggleable)
- Triggered only if `content_llm_enabled` is true AND the user taps "Refine with AI"
  on the review screen (never automatic — FR18).
- Prompt (server-side, assembled): system = brand voice + GEO context; user = the
  template-generated title/description + slab facts; task = "improve the title and
  description for a premium one-of-a-one wood slab listing. Keep all measurements
  exact. Do not invent features. Return JSON: {title, short_title, description}."
- **Guardrail (deterministic)**: after the LLM returns, the server re-checks that every
  numeric dimension in the LLM text still matches the pipeline values (regex on
  numbers). Mismatch → reject the LLM output, keep the template version, log the
  rejection. This keeps measurement accuracy deterministic even when the LLM is on.
...
- Deterministic: templates, all dimension text, category/tag/attribute resolution,
  payload assembly, dedupe, logging.
- Scoped inference: optional LLM title/description refinement only, with the
  numeric-accuracy guardrail (§1.2).
